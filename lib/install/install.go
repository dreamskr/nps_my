...
func downloadLatest(bin string) string {
	// get version
	data, err := http.Get("https://api.github.com/repos/dreamskr/nps/releases/latest")
	if err != nil {
		log.Fatal(err.Error())
	}
	...
	downloadUrl := fmt.Sprintf("https://github.com/dreamskr/nps/releases/download/%s/%s", version, filename)
